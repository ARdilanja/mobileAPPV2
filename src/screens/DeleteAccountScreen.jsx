...
const DeleteAccountScreen = ({ navigation }) => {
  ...
  const onDeletePress = () => {
    if (selectedReason.length === 0) {
      Alert.alert('Required', 'Please select at least one reason to continue.');
      return;
    }
    setShowModal(true);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      let reasons = selectedReason.filter(r => r !== 'Others');
      if (selectedReason.includes('Others') && otherText.trim()) {
        reasons.push(`Others: ${otherText.trim()}`);
      }
      if (reasons.length === 0) reasons = ['No reason provided'];
      ...
      // 🔴 DEBUG: Console log full request details
      console.log('🗑️ DELETE ACCOUNT REQUEST');
      console.log('Endpoint:', `${API_BASE}/auth/delete-account`);
      ...
      Alert.alert(
        ...
        error.response?.data?.message ||
          'Failed to process request. Check your connection or try again.',
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {/* CHECKBOX LIST - EXACTLY AS ORIGINAL */}
      ...
      <TouchableOpacity
        style={[
          styles.deleteBtn,
          selectedReason.length === 0 && { opacity: 0.6 },
        ]}
      ...
      <Modal visible={showModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity
              style={[styles.confirmBtn, isDeleting && { opacity: 0.7 }]}
